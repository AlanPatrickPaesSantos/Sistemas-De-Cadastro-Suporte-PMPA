        ...
        <div className="flex flex-col md:flex-row gap-4 relative">
          <div className="md:w-64 space-y-1.5">
            <div className="flex justify-between items-center px-1">
              <label className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest">Unidade</label>
              {selectedUnidade && (
                <button 
                  onClick={() => setSelectedUnidade("")} 
            ...
            <UnidadeCombobox value={selectedUnidade} onChange={setSelectedUnidade} />
          </div>
          <div className="relative flex-1 group/input flex flex-col justify-end">
            <div className="flex justify-between items-center px-1 mb-1.5 md:hidden">
              <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Busca Rápida</label>
            </div>
            <div className="relative">
              ...
              <Input
                ...
                placeholder={filterType === 'all' ? "ID, Série, RP, Solicitante..." : "Digite o termo..."}
                value={query}
                onChange={(e) => setQuery(e.target.value)}
                className="pl-12 pr-12 h-10 md:h-10 text-sm font-medium border-slate-200 dark:border-slate-700 focus-visible:ring-1 focus-visible:ring-[#004e9a]/30 focus-visible:border-[#004e9a] shadow-sm w-full rounded-xl bg-slate-50 dark:bg-slate-800 transition-all text-slate-800 dark:text-slate-100 placeholder:text-slate-400"
              />
              <Select value={filterType} onValueChange={setFilterType}>
                <SelectTrigger className="absolute right-2 top-1/2 -translate-y-1/2 h-8 w-8 p-0 flex items-center justify-center border-none bg-transparent hover:bg-slate-200 dark:hover:bg-slate-700/50 focus:ring-0 shadow-none transition-colors rounded-lg">
                ...
                <SelectContent className="rounded-xl border-slate-200 shadow-xl backdrop-blur-xl bg-white/95 z-[100] mt-1 mr-2">
                  <SelectItem value="all" className="font-bold text-[10px] py-2 uppercase text-slate-700 focus:bg-blue-50 focus:text-[#004e9a] cursor-pointer">🌐 TUDO</SelectItem>
                  <SelectItem value="os" className="font-bold text-[10px] py-2 uppercase text-slate-700 focus:bg-blue-50 focus:text-[#004e9a] cursor-pointer">🔢 OS</SelectItem>
                  <SelectItem value="serie" className="font-bold text-[10px] py-2 uppercase text-slate-700 focus:bg-blue-50 focus:text-[#004e9a] cursor-pointer">🔤 SÉRIE</SelectItem>
                  <SelectItem value="rp" className="font-bold text-[10px] py-2 uppercase text-slate-700 focus:bg-blue-50 focus:text-[#004e9a] cursor-pointer">🏷️ RP</SelectItem>
                  <SelectItem value="unidade" className="font-bold text-[10px] py-2 uppercase text-slate-700 focus:bg-blue-50 focus:text-[#004e9a] cursor-pointer">🏢 UNIDADE</SelectItem>
                </SelectContent>
              </Select>
            </div>
          </div>
